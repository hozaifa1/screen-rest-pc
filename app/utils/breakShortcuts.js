...
function onShortcut ({ name, settings, log, breakPlanner, functions }) {
  switch (name) {
    ...
    case 'skipToNextScheduledBreakShortcut':
      log.info('ScreenRest: skipping to next scheduled break by shortcut')
    ...
    case 'skipToNextMiniBreakShortcut':
      log.info('ScreenRest: skipping to next Mini break by shortcut')
      functions.skipToMicrobreak()
      break
    case 'skipToNextLongBreakShortcut':
      log.info('ScreenRest: skipping to next Long break by shortcut')
      functions.skipToBreak()
      break
    case 'resetBreaksShortcut':
      log.info('ScreenRest: resetting breaks by shortcut')
...
function setupBreak ({ name, shortcutText, settings, log, globalShortcut, breakPlanner, functions }) {
  ...
  if (shortcut) {
    log.info(`ScreenRest: ${name} registration successful (${shortcutText})`)
  } else {
    log.warn(`ScreenRest: ${name} registration failed`)
  }
}
