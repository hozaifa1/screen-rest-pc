# ScreenRest [![Awesome Humane Tech](https://raw.githubusercontent.com/humanetech-community/awesome-humane-tech/main/humane-tech-badge.svg?sanitize=true)](https://github.com/humanetech-community/awesome-humane-tech) [![Build Status](https://travis-ci.org/hozaifa1/screenrest-pc.svg?branch=master)](https://travis-ci.org/hozaifa1/screenrest-pc) [![JavaScript Style Guide](https://img.shields.io/badge/code%20style-standard-brightgreen.svg)](http://standardjs.com/) [![codecov](https://codecov.io/gh/hozaifa1/screenrest-pc/graph/badge.svg?token=eOjky4kr2j)](https://codecov.io/gh/hozaifa1/screenrest-pc) [![Translation status](https://hosted.weblate.org/widgets/ScreenRest/-/ScreenRest/svg-badge.svg)](https://hosted.weblate.org/engage/ScreenRest/)

<img src="ScreenRest_128x128.png" align="right" alt="ScreenRest logo">

> **The break time reminder app**

*ScreenRest* is a cross-platform [Electron](https://www.electronjs.org/) app that reminds you to take breaks when working on your computer.

[![Become a Sponsor!](https://img.shields.io/static/v1?label=Sponsor&message=%E2%9D%A4&logo=GitHub&color=success)](https://github.com/sponsors/hozaifa1) [![Become a Patron!](https://img.shields.io/static/v1?label=Sponsor&message=%E2%9D%A4&logo=Patreon&color=success)](https://www.patreon.com/hozaifa1)
...
## Install [![GitHub All Releases](https://img.shields.io/github/downloads/hozaifa1/screenrest-pc/total)](https://github.com/hozaifa1/screenrest-pc/releases) [![Packaging status](https://repology.org/badge/tiny-repos/ScreenRest.svg)](https://repology.org/project/ScreenRest/versions)

The latest official **installers** and **portable versions** for macOS, Windows and Linux can be found at [Github Releases](https://github.com/hozaifa1/screenrest-pc/releases) page. For supported versions of your OS check [Electron's supported versions](https://github.com/electron/electron/blob/main/README.md#platform-support). Read on for details of specific OSes.

### macOS

It is recommended to install *ScreenRest* with [Homebrew](https://brew.sh/) by running the folowing command.
See [Application Signing](#application-signing) for details.
```zsh
brew update && brew install --cask --no-quarantine ScreenRest
```

When upgrading, run the following command.
Don't forget to Quit ScreenRest, first.
```zsh
brew update && brew upgrade --cask
```

If you're using [Alfred](https://www.alfredapp.com) or [Raycast](https://www.raycast.com/) on macOS you can use this [Alfred Workflow](https://github.com/KingOfSpades/stretchFred) or [Raycast Extension](https://www.raycast.com/u-ran/ScreenRest) to interact with ScreenRest.

#### Application Signing

*ScreenRest* is not signed (due to its costs and me not owning supported Apple device) so you will need to use a workaround for the first run. The workaround
...
```bash
sudo xattr -r -d com.apple.quarantine /Applications/ScreenRest.app
```
...
```bash
brew install --cask --no-quarantine ScreenRest
```
...
You can also install *ScreenRest* with [Chocolatey](https://chocolatey.org) by running the following command from the command line or from PowerShell:
```powershell
choco install ScreenRest
```

To update:
```powershell
choco upgrade ScreenRest
```

ScreenRest is also available in Microsoft's [winget](https://docs.microsoft.com/en-us/windows/package-manager/winget/).
You can install it by running:
```powershell
winget install -e --id ScreenRest.ScreenRest
```

ScreenRest can be also found at official [Microsoft Store](https://apps.microsoft.com/store/detail/ScreenRest/9PP2B76LQQBN?hl=en-us&gl=us).

You can install *ScreenRest* for all users silently by running this as administrator:
```cmd
...
For portable versions and for Linux installers, head to [Github Releases page](https://github.com/hozaifa1/screenrest-pc/releases). The most widely used distributions should be covered.

ScreenRest is also available in some of the Linux stores:
- [Flathub](https://flathub.org/apps/details/net.hozaifa1.ScreenRest)
- [Snap Store](https://snapcraft.io/ScreenRest)

For Debian/Ubuntu and derivates you could also try this `apt` repository: `deb [trusted=yes] https://apt.fury.io/hozaifa1/ /`.
...
If *ScreenRest* is not starting, you might need to run:
```sh
...
To run *ScreenRest* from source you will need [Node.js](https://nodejs.org/), ideally the one specified in `package.json`. Clone the repo, run `npm install` and then simply run `npm start` to start *ScreenRest*.
...
When you run *ScreenRest* for the first time, you are presented with a Welcome window that allows you to change the language, review the preferences, view the online tutorial or simply continue with the default preferences.

<img src="welcome.png" height="340">

*ScreenRest* itself lives in your tray, only displaying a reminder window from time to time, which contains an idea for a break.
...
Clicking the *ScreenRest* icon in your tray area will display the current status of breaks, provide menu items with extra functionality, and link to the Preferences.

<img src="tray.png" height="140">

*ScreenRest* is monitoring your idle time, so when you are idle for 5 minutes, breaks will be paused until you return.

*ScreenRest* is also monitoring Do Not Disturb mode, so breaks are paused when DnD mode is On.

*ScreenRest* follows the theme of your system and is also available in dark mode.

<img src="dark.png" height="340">

### Interact with ScreenRest from command line

When a ScreenRest instance is running, the `ScreenRest` command can be use to interact with it from the command line.

Type `ScreenRest help` to get a list of all commands and options available as well as some examples.
...
Preferences are divided into multiple categories and you are encouraged to take some time to make *ScreenRest* your own by customizing them.
...
All preferences are saved in a JSON file, so you can set ScreenRest in the way you need. Use the `Ctrl/Cmd + D` shortcut while viewing the About section of Preferences, to show debug info and display a clickable link to the preferences file.

It's recommended to Quit *ScreenRest* before editing the preferences file.
...
Preferences marked with ![Not Reliable](https://img.shields.io/badge/Not_Reliable-β-yellow) might not work correctly and might break *ScreenRest*. Use at own risk.
...
Note that when a new *ScreenRest* version with new break ideas is out, your custom ideas will not be overwritten. You can reset break ideas to the latest defaults when you "Restore defaults" from Preferences window.
...
Place your images in the `images` folder inside ScreenRest's user data directory (see Preferences → About → Debug info for the exact location). Then reference them by filename only:
...
ScreenRest sanitizes all HTML to keep break windows secure by removing any unsupported tags or unsafe content.
...
In the preferences file, set `naturalBreaksInactivityResetTime` to your preferred value (in milliseconds greater than than 20000ms). This is an idle time length, after which *ScreenRest* breaks will be paused until the user resumes activity.
...
#### Make ScreenRest show breaks as regular windows [![Contributor Preferences](https://img.shields.io/badge/Contributor_Preferences-✔-success)](#contributor-preferences) ![Not Reliable](https://img.shields.io/badge/Not_Reliable-β-yellow)

If you want ScreenRest breaks to act as regular windows (have a titlebar, turn off always on top, be minimizable and focusable) set `showBreaksAsRegularWindows` to `true`.

#### Pause/resume breaks only when specific command is running

By editing `appExclusions` in preferences file, you can automatically control when ScreenRest breaks are paused.

If you want ScreenRest to be paused when specific apps are running, you could have this value (breaks are paused when Skype or Atom are running):
...
If you want ScreenRest to be running when specific apps are as well, you could have this value (breaks are paused when Skype or Atom are not running):
...
To hide ScreenRest icon in menubar/tray, set the value of `showTrayIcon` from `true` to `false`.

Note that this will disable graphical way of opening ScreenRest Preferences. To access Preferences, you will have to use command line options (ie: `ScreenRest preferences` on Linux).
...
If you want to disable functionality around app updates, set `disableAppUpdateFeatures` to `true`. This will make ScreenRest not to check for new versions and hide related elements from the app. This value takes preference over `checkNewVersion` and `notifyNewVersion`.
...
*ScreenRest* is free but you can support it by contributing code, translations or money. You will be rewarded by getting access to **Contributor Preferences**, ability to **Sync Preferences**, chat on **Discord** and more!

You can access contributor features after authenticating with  Github or Patreon in the "Love ScreenRest" section of Preferences.

<img src="contributors.png" height="340"/>

### [Sponsoring on Github](https://github.com/sponsors/hozaifa1) [![Become a Sponsor!](https://img.shields.io/static/v1?label=Sponsor&message=%E2%9D%A4&logo=GitHub&color=success)](https://github.com/sponsors/hozaifa1)
...
### [Sponsoring on Patreon](https://www.patreon.com/hozaifa1) [![Become a Patron!](https://img.shields.io/static/v1?label=Sponsor&message=%E2%9D%A4&logo=Patreon&color=success)](https://www.patreon.com/hozaifa1)
...
### [Donating with PayPal](https://paypal.me/Janhozaifa1) [![Donate with PayPal](https://img.shields.io/static/v1?label=Donate&message=%E2%9D%A4&logo=Paypal&color=success)](https://paypal.me/Janhozaifa1)
PayPal has fees.
...
## Development
Feel free to join in the [development](https://github.com/hozaifa1/screenrest-pc/blob/master/CONTRIBUTING.md) of this app via Issues and Pull Requests.
...
Now you can clone the repo with `git clone https://github.com/hozaifa1/screenrest-pc.git`. Change to the new created directory and run `npm i` to install node packages needed. If your system is running Python 3.12 and newer, you also need to install `setuptools`, for example with `pip install setuptools`, as `distutils` have been [deprecated](https://docs.python.org/3/library/distutils.html).

Read on.

### Debugging

You can use ScreenRest's built-in debug shortcut by pressing `Ctrl/Cmd + D` in the About section to show information such as:
...
If you start *ScreenRest* in development mode with the `npm run dev` command, it makes it possible to debug the application in your browser on `http://localhost:9222`.

### Logging

*ScreenRest* uses `[log](https://github.com/megahertz/electron-log)` package for some extra logging.
Format as following:
- `System: my message` for messages regarding Operating System, ie: `System: resume or unlock`
- `ScreenRest: my message` for messages regarding *ScreenRest*

### Translations

You can help to translate ScreenRest on [Weblate](https://hosted.weblate.org/engage/ScreenRest). Consider joining with your Github account to be correctly credited.
[![Translation status](https://hosted.weblate.org/widgets/ScreenRest/-/ScreenRest/horizontal-auto.svg)](https://hosted.weblate.org/engage/ScreenRest/)


## Known issues
- Autostart does not work in Flathub app ([#1517](https://github.com/hozaifa1/screenrest-pc/issues/1517))
...
- Wayland multi-display window placement issue puts all break windows on one monitor; start with X11 backend (`ScreenRest --ozone-platform=x11`) if needed. See [electron/electron#48749](https://github.com/electron/electron/issues/48749).

### MacOS
- users experiencing their Dock hiding after a break, requiring command + tab or a mouse click to get focus back, check System Preferences > Users & Groups > {User} > Login Items. If Hide is checked for ScreenRest, uncheck it, it should solve the issue.
...
- users with Chromium-based browser (Chrome, Edge, etc...) [might need to set some flags](https://github.com/hozaifa1/screenrest-pc/issues/783#issuecomment-762819646) when browser is non-responsive after break

## Contributors

- Jan hozaifa1, @hozaifa1, [github.com/hozaifa1](https://github.com/hozaifa1)
- Martina Mocinecova, (pre-1.0 *ScreenRest* logo), color schemes
...
Also see Github's list of [contributors](https://github.com/hozaifa1/screenrest-pc/graphs/contributors).
...
#### Sounds credits
Sounds used in this application are listed [here](http://freesound.org/people/hozaifa1/bookmarks/category/58865/).
...
## License
See [LICENSE](https://github.com/hozaifa1/screenrest-pc/blob/master/LICENSE) file.
